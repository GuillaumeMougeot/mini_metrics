...
def split_cols(
    df: pd.DataFrame, pattern: str | re.Pattern | Iterable[str | re.Pattern]
):
...
def plot_df(
    ...
    for gi, grp in enumerate(groups):
        ...
        for col in metrics:
            ...
            pts.set_offsets(
                [(x, y + yoff) for i, (x, y) in enumerate(pts.get_offsets())]
            )
...
def cli():
    parser = ArgumentParser(
        prog="mini_metric_plot", description="Plot and compare multiple mini_metrics"
    )
    parser.add_argument(
        "-i",
        "--input",
